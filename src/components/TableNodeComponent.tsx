...
export const TableNodeComponent = memo(
  (props: ExtendedTableNodeProps) => {
    ...
    const onEdit =
      propOnEdit || (props.data as TableNode & { onEdit?: (tableId: string) => void })?.onEdit;
    ...
    return (
      ...
      >
        ...
        {/* Table Fields */}
        <div
          className={`${cardBgColor} text-white`}
          style={{ backgroundColor: color ? undefined : '#0f172a' }}
        >
          {fields.map((field, index) => (
            <div
              key={index}
              className="field-row flex items-center py-2 px-3 border-b border-slate-700/30 last:border-0 relative"
            >
              ...
              {/* Field name */}
              <div className="field-name flex-1 font-medium text-sm truncate max-w-[100px]">
                {field.name}
              </div>
              {/* Field type badge */}
...
const areFieldsEqual = (prev: Field[], next: Field[]): boolean => {
  ...
  return prev.every(
    (field, index) =>
      ...
      field.primary === next[index].primary &&
      field.notNull === next[index].notNull &&
      field.foreignKey?.tableId === next[index].foreignKey?.tableId &&
      field.foreignKey?.fieldName === next[index].foreignKey?.fieldName
  );
};
